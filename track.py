#!/usr/bin/env python3
"""Run WALDO + ByteTrack/BotSORT multi-object tracking on a video/stream, live by default.

Defaults to --weights waldo-l-p2 --show, on this box's GPU (a Jetson Orin NX,
not the Pi 5 the onboard adts/ package targets - see WALDO_WEIGHTS below for
the accuracy/speed tradeoffs between checkpoints). GPU-only: WALDO only ships
.pt checkpoints, no NCNN export for a CPU preview path.
...
# detector: fast, confident LightVehicle/Truck/Bus detection, but weak
# Person recall on close-range rescue scenes (0 detections at conf=0.25 in
# testing; best candidate was only 0.18 even at conf=0.05) - it was trained
# on general overhead imagery, not fine-tuned for close-range rescue scenes.
# Use for vehicle/infrastructure awareness; treat a Person detection as a
# lower-confidence signal, not a reliable human-detection guarantee.
# GPU-only - only .pt checkpoints were pulled, no NCNN export made.
WALDO_WEIGHTS = {
...
def main():
    ...
    parser.add_argument(
        "--weights",
        choices=sorted(WALDO_WEIGHTS.keys()),
        default="waldo-l-p2",
        help="which WALDO checkpoint to use (default: waldo-l-p2 - see WALDO_WEIGHTS comment above).",
    )
    parser.add_argument(
        "--lang",
        choices=sorted(WALDO_NAMES.keys()),
        default="tr",
        help="label language for WALDO's class names (see WALDO_NAMES above). en/fr/de/es/pt/tr stay "
             "ASCII-transliterated for the fast cv2 text path; ru/uk/ar/zh use real Cyrillic/Arabic/Chinese "
             "script, which forces the slower PIL text-rendering path (cv2.putText's built-in fonts are "
             "ASCII-only) - expect a bigger FPS hit from those than from the language choice itself.",
    )
    parser.add_argument(
        "--source",
    ...
    parser.add_argument("--exclude-classes", default="UPole",
                         help="comma-separated class names to drop before tracking (matched against WALDO's own names); "
                              "empty string = track every class")
    ...
    parser.add_argument("--roi", type=float, default=0.0,
                         help="crop each frame to this fraction of width/height, centered, before detecting/tracking/displaying (e.g. "
                              ...
                              "decode/resize is a genuine speed win, and the same real-world object also fills more of the model's "
                              "input after cropping, which helps small-object recall")
    parser.add_argument("--half", action=argparse.BooleanOptionalAction, default=True,
                         help="FP16 inference (default: on). Benchmarked on waldo-m: 56.9ms FP32 -> 40.4ms FP16, ~29%% faster, "
                              "for free (no accuracy tradeoff noted)")
    ...
    print(f"Using {weights_path.name}")
    ...
    label_font = None if all(name.isascii() for name in lang_table.values()) else get_unicode_font()

    if not args.show:
        results = (
            model.track(
                source=args.source, imgsz=args.imgsz, conf=args.conf, device=0,
                tracker=args.tracker, classes=class_ids, half=args.half, persist=True, stream=True,
            ) if args.track else
            model.predict(
                source=args.source, imgsz=args.imgsz, conf=args.conf, device=0,
                classes=class_ids, half=args.half, stream=True,
            )
        )
    ...
    if args.show:
        ...
        controls += ", q=quit"
        window_name = f"WALDO track ({args.weights}, {mode_label}) - {controls}"
        cv2.namedWindow(window_name, cv2.WINDOW_NORMAL)
        selection = TrackSelection()
        cv2.setMouseCallback(window_name, selection.on_mouse)
        predict_kwargs = dict(imgsz=args.imgsz, conf=args.conf, device=0,
                               classes=class_ids, half=args.half, verbose=False)
