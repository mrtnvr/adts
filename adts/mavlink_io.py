...
vehicle's system ID, and uses the standard camera tracking protocol:
  ...
  in:  MAV_CMD_CAMERA_TRACK_POINT      (2004) p1,p2 = x,y normalised 0..1   -> lock the detection there
       ...
       MAV_CMD_USER_1                  (31010) change target (ADTS-specific, standard MAVLink has no command for it):
                                          p1 = +1 next / -1 prev (left-to-right order)
                                          p1 = 0 and p2 = track ID -> select that ID
...
TRACK_CMDS = {M.MAV_CMD_CAMERA_TRACK_POINT, M.MAV_CMD_CAMERA_TRACK_RECTANGLE,
              M.MAV_CMD_CAMERA_STOP_TRACKING, M.MAV_CMD_USER_1}
...
class MavlinkIO:
    ...
    def _on_command(self, msg):
        ...
        if cmd == M.MAV_CMD_CAMERA_TRACK_POINT:
            c = MavCommand("point", (msg.param1 * w, msg.param2 * h), ack)
        elif cmd == M.MAV_CMD_CAMERA_TRACK_RECTANGLE:
            c = MavCommand("rect", (msg.param1 * w, msg.param2 * h, msg.param3 * w, msg.param4 * h), ack)
        elif cmd == M.MAV_CMD_CAMERA_STOP_TRACKING:
            c = MavCommand("stop", None, ack)
        elif int(msg.param1) in (1, -1):
            c = MavCommand("cycle", int(msg.param1), ack)
        elif int(msg.param1) == 2:
            c = MavCommand("auto", None, ack)
        else:
            c = MavCommand("select", int(msg.param2), ack)
        self.commands.put(c)
